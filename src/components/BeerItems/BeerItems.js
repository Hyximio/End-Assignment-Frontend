...
function BeerItems( {targetBeer} ) {

    let { beerData } = useContext( BeerDataContext );

    if (targetBeer){
        const potentialBeers = getPotentialBeerByBeer( beerData, targetBeer );

        beerData = []
        for( let i = 1; i < 7; i++){
            beerData.push( potentialBeers[i])
        }
    ...
    const beerItems = beerData.map( (beer) =>
        ...
        <Link to={`/beers/${beer.id-1}`} key={beer.name} className="beer-item" >
